/*******************************************************************************
* File Name: ConsolePort_2_Clock_2.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: ConsolePort_2_Clock_2_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_2_Clock_2_Start(void) 
{
    /* Set the bit to enable the clock. */
    ConsolePort_2_Clock_2_CLKEN |= ConsolePort_2_Clock_2_CLKEN_MASK;
	ConsolePort_2_Clock_2_CLKSTBY |= ConsolePort_2_Clock_2_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ConsolePort_2_Clock_2_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_2_Clock_2_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    ConsolePort_2_Clock_2_CLKEN &= (uint8)(~ConsolePort_2_Clock_2_CLKEN_MASK);
	ConsolePort_2_Clock_2_CLKSTBY &= (uint8)(~ConsolePort_2_Clock_2_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ConsolePort_2_Clock_2_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_2_Clock_2_StopBlock(void) 
{
    if ((ConsolePort_2_Clock_2_CLKEN & ConsolePort_2_Clock_2_CLKEN_MASK) != 0u)
    {
...
#if defined(ConsolePort_2_Clock_2__CFG3)
        CLK_DIST_AMASK = ConsolePort_2_Clock_2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ConsolePort_2_Clock_2_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ConsolePort_2_Clock_2__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ConsolePort_2_Clock_2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ConsolePort_2_Clock_2_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ConsolePort_2_Clock_2_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_2_Clock_2_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        ConsolePort_2_Clock_2_CLKSTBY &= (uint8)(~ConsolePort_2_Clock_2_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: ConsolePort_2_Clock_2_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_2_Clock_2_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = ConsolePort_2_Clock_2_GetSourceRegister();
    uint16 oldDivider = ConsolePort_2_Clock_2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ConsolePort_2_Clock_2_CLKEN & ConsolePort_2_Clock_2_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ConsolePort_2_Clock_2_DIV_PTR, clkDivider);
                ConsolePort_2_Clock_2_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ConsolePort_2_Clock_2_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ConsolePort_2_Clock_2_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ConsolePort_2_Clock_2_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ConsolePort_2_Clock_2_DIV_PTR, clkDivider);
				ConsolePort_2_Clock_2_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: ConsolePort_2_Clock_2_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 ConsolePort_2_Clock_2_GetDividerRegister(void) 
{
    return CY_GET_REG16(ConsolePort_2_Clock_2_DIV_PTR);
}


/*******************************************************************************
* Function Name: ConsolePort_2_Clock_2_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_2_Clock_2_SetModeRegister(uint8 modeBitMask) 
{
    ConsolePort_2_Clock_2_MOD_SRC |= modeBitMask & (uint8)ConsolePort_2_Clock_2_MODE_MASK;
}


/*******************************************************************************
* Function Name: ConsolePort_2_Clock_2_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_2_Clock_2_ClearModeRegister(uint8 modeBitMask) 
{
    ConsolePort_2_Clock_2_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ConsolePort_2_Clock_2_MODE_MASK));
}


/*******************************************************************************
* Function Name: ConsolePort_2_Clock_2_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ConsolePort_2_Clock_2_GetModeRegister(void) 
{
    return ConsolePort_2_Clock_2_MOD_SRC & (uint8)(ConsolePort_2_Clock_2_MODE_MASK);
}


/*******************************************************************************
* Function Name: ConsolePort_2_Clock_2_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_2_Clock_2_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = ConsolePort_2_Clock_2_GetDividerRegister();
    uint8 oldSrc = ConsolePort_2_Clock_2_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ConsolePort_2_Clock_2_MOD_SRC |= CYCLK_SSS;
        ConsolePort_2_Clock_2_MOD_SRC =
            (ConsolePort_2_Clock_2_MOD_SRC & (uint8)(~ConsolePort_2_Clock_2_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ConsolePort_2_Clock_2_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ConsolePort_2_Clock_2_GetSourceRegister(void) 
{
    return ConsolePort_2_Clock_2_MOD_SRC & ConsolePort_2_Clock_2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ConsolePort_2_Clock_2_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_2_Clock_2_SetPhaseRegister(uint8 clkPhase) 
{
    ConsolePort_2_Clock_2_PHASE = clkPhase & ConsolePort_2_Clock_2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ConsolePort_2_Clock_2_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ConsolePort_2_Clock_2_GetPhaseRegister(void) 
{
    return ConsolePort_2_Clock_2_PHASE & ConsolePort_2_Clock_2_PHASE_MASK;
}
