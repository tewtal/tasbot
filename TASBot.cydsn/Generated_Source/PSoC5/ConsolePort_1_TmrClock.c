/*******************************************************************************
* File Name: ConsolePort_1_TmrClock.c
* Version 2.20
*
*  Description:
...
#include "ConsolePort_1_TmrClock.h"
...
/*******************************************************************************
* Function Name: ConsolePort_1_TmrClock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_1_TmrClock_Start(void) 
{
    /* Set the bit to enable the clock. */
    ConsolePort_1_TmrClock_CLKEN |= ConsolePort_1_TmrClock_CLKEN_MASK;
	ConsolePort_1_TmrClock_CLKSTBY |= ConsolePort_1_TmrClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ConsolePort_1_TmrClock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_1_TmrClock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    ConsolePort_1_TmrClock_CLKEN &= (uint8)(~ConsolePort_1_TmrClock_CLKEN_MASK);
	ConsolePort_1_TmrClock_CLKSTBY &= (uint8)(~ConsolePort_1_TmrClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ConsolePort_1_TmrClock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_1_TmrClock_StopBlock(void) 
{
    if ((ConsolePort_1_TmrClock_CLKEN & ConsolePort_1_TmrClock_CLKEN_MASK) != 0u)
    {
...
#if defined(ConsolePort_1_TmrClock__CFG3)
        CLK_DIST_AMASK = ConsolePort_1_TmrClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ConsolePort_1_TmrClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ConsolePort_1_TmrClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ConsolePort_1_TmrClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ConsolePort_1_TmrClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ConsolePort_1_TmrClock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_1_TmrClock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        ConsolePort_1_TmrClock_CLKSTBY &= (uint8)(~ConsolePort_1_TmrClock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: ConsolePort_1_TmrClock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_1_TmrClock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = ConsolePort_1_TmrClock_GetSourceRegister();
    uint16 oldDivider = ConsolePort_1_TmrClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ConsolePort_1_TmrClock_CLKEN & ConsolePort_1_TmrClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ConsolePort_1_TmrClock_DIV_PTR, clkDivider);
                ConsolePort_1_TmrClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ConsolePort_1_TmrClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ConsolePort_1_TmrClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ConsolePort_1_TmrClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ConsolePort_1_TmrClock_DIV_PTR, clkDivider);
				ConsolePort_1_TmrClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: ConsolePort_1_TmrClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 ConsolePort_1_TmrClock_GetDividerRegister(void) 
{
    return CY_GET_REG16(ConsolePort_1_TmrClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: ConsolePort_1_TmrClock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_1_TmrClock_SetModeRegister(uint8 modeBitMask) 
{
    ConsolePort_1_TmrClock_MOD_SRC |= modeBitMask & (uint8)ConsolePort_1_TmrClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: ConsolePort_1_TmrClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_1_TmrClock_ClearModeRegister(uint8 modeBitMask) 
{
    ConsolePort_1_TmrClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ConsolePort_1_TmrClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: ConsolePort_1_TmrClock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ConsolePort_1_TmrClock_GetModeRegister(void) 
{
    return ConsolePort_1_TmrClock_MOD_SRC & (uint8)(ConsolePort_1_TmrClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: ConsolePort_1_TmrClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_1_TmrClock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = ConsolePort_1_TmrClock_GetDividerRegister();
    uint8 oldSrc = ConsolePort_1_TmrClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ConsolePort_1_TmrClock_MOD_SRC |= CYCLK_SSS;
        ConsolePort_1_TmrClock_MOD_SRC =
            (ConsolePort_1_TmrClock_MOD_SRC & (uint8)(~ConsolePort_1_TmrClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ConsolePort_1_TmrClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ConsolePort_1_TmrClock_GetSourceRegister(void) 
{
    return ConsolePort_1_TmrClock_MOD_SRC & ConsolePort_1_TmrClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ConsolePort_1_TmrClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ConsolePort_1_TmrClock_SetPhaseRegister(uint8 clkPhase) 
{
    ConsolePort_1_TmrClock_PHASE = clkPhase & ConsolePort_1_TmrClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ConsolePort_1_TmrClock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ConsolePort_1_TmrClock_GetPhaseRegister(void) 
{
    return ConsolePort_1_TmrClock_PHASE & ConsolePort_1_TmrClock_PHASE_MASK;
}
