/*******************************************************************************
* File Name: ConsolePort_2_TmrClock.c
* Version 2.20
*
*  Description:
...
#include "ConsolePort_2_TmrClock.h"
...
/*******************************************************************************
* Function Name: ConsolePort_2_TmrClock_Start
********************************************************************************
*
* Summary:
...
void ConsolePort_2_TmrClock_Start(void) 
{
    ...
    ConsolePort_2_TmrClock_CLKEN |= ConsolePort_2_TmrClock_CLKEN_MASK;
	ConsolePort_2_TmrClock_CLKSTBY |= ConsolePort_2_TmrClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ConsolePort_2_TmrClock_Stop
********************************************************************************
*
* Summary:
...
void ConsolePort_2_TmrClock_Stop(void) 
{
    ...
    ConsolePort_2_TmrClock_CLKEN &= (uint8)(~ConsolePort_2_TmrClock_CLKEN_MASK);
	ConsolePort_2_TmrClock_CLKSTBY &= (uint8)(~ConsolePort_2_TmrClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ConsolePort_2_TmrClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ConsolePort_2_TmrClock_CLKEN & ConsolePort_2_TmrClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ConsolePort_2_TmrClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ConsolePort_2_TmrClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ConsolePort_2_TmrClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ConsolePort_2_TmrClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ConsolePort_2_TmrClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ConsolePort_2_TmrClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ConsolePort_2_TmrClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ConsolePort_2_TmrClock_GetSourceRegister();
    uint16 oldDivider = ConsolePort_2_TmrClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ConsolePort_2_TmrClock_CLKEN & ConsolePort_2_TmrClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ConsolePort_2_TmrClock_DIV_PTR, clkDivider);
                ConsolePort_2_TmrClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ConsolePort_2_TmrClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ConsolePort_2_TmrClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ConsolePort_2_TmrClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ConsolePort_2_TmrClock_DIV_PTR, clkDivider);
				ConsolePort_2_TmrClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: ConsolePort_2_TmrClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ConsolePort_2_TmrClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: ConsolePort_2_TmrClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ConsolePort_2_TmrClock_MOD_SRC |= modeBitMask & (uint8)ConsolePort_2_TmrClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: ConsolePort_2_TmrClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ConsolePort_2_TmrClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ConsolePort_2_TmrClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: ConsolePort_2_TmrClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ConsolePort_2_TmrClock_MOD_SRC & (uint8)(ConsolePort_2_TmrClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: ConsolePort_2_TmrClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ConsolePort_2_TmrClock_GetDividerRegister();
    uint8 oldSrc = ConsolePort_2_TmrClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ConsolePort_2_TmrClock_MOD_SRC |= CYCLK_SSS;
        ConsolePort_2_TmrClock_MOD_SRC =
            (ConsolePort_2_TmrClock_MOD_SRC & (uint8)(~ConsolePort_2_TmrClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ConsolePort_2_TmrClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ConsolePort_2_TmrClock_MOD_SRC & ConsolePort_2_TmrClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ConsolePort_2_TmrClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ConsolePort_2_TmrClock_PHASE = clkPhase & ConsolePort_2_TmrClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ConsolePort_2_TmrClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ConsolePort_2_TmrClock_PHASE & ConsolePort_2_TmrClock_PHASE_MASK;
}
