...
uint8 P1_2_initVar = 0u;


/*******************************************************************************
* Function Name: P1_2_Start
********************************************************************************
*
* Summary:
...
*
* Global Variables:
*  P1_2_initVar - used to check initial configuration, modified on
*  first function call.
*
* Reentrant:
*  No.
...
void P1_2_Start(void) 
{
    ...
    {
        P1_2_Init();
        P1_2_initVar = 1u; /* Component initialized */
    }
...
void P1_2_Init(void) 
{
    P1_2_SetIntMode(P1_2_INT_SRC);
}


/*******************************************************************************
* Function Name: P1_2_Stop
********************************************************************************
*
* Summary:
...
void P1_2_Stop(void) 
{
    ...
    P1_2_SR_CONTROL &= ((uint8) ~P1_2_CLK_EN);
    P1_2_DisableInt();
}


/*******************************************************************************
* Function Name: P1_2_EnableInt
********************************************************************************
*
* Summary:
...
void P1_2_EnableInt(void) 
{
    ...
    P1_2_SR_AUX_CONTROL |= P1_2_INTERRUPTS_ENABLE;
    CyExitCriticalSection(interruptState);
}


/*******************************************************************************
* Function Name: P1_2_DisableInt
********************************************************************************
*
* Summary:
...
void P1_2_DisableInt(void) 
{
    ...
    interruptState = CyEnterCriticalSection();
    P1_2_SR_AUX_CONTROL &= ((uint8) ~P1_2_INTERRUPTS_ENABLE);
    CyExitCriticalSection(interruptState);
}


/*******************************************************************************
* Function Name: P1_2_GetFIFOStatus
********************************************************************************
*
* Summary:
...
uint8 P1_2_GetFIFOStatus(uint8 fifoId) 
{
    ...
    result = P1_2_RET_FIFO_NOT_DEFINED;

    #if(0u != P1_2_USE_INPUT_FIFO)
        if(P1_2_IN_FIFO == fifoId)
        {
            switch(P1_2_GET_IN_FIFO_STS)
            {
                case P1_2_IN_FIFO_FULL :
                    result = P1_2_RET_FIFO_FULL;
                    break;

                case P1_2_IN_FIFO_EMPTY :
                    result = P1_2_RET_FIFO_EMPTY;
                    break;

                case P1_2_IN_FIFO_PARTIAL:
                    result = P1_2_RET_FIFO_PARTIAL;
                    break;
    ...
    if(P1_2_OUT_FIFO == fifoId)
    {
        switch(P1_2_GET_OUT_FIFO_STS)
        {
            case P1_2_OUT_FIFO_FULL :
                result = P1_2_RET_FIFO_FULL;
                break;

            case P1_2_OUT_FIFO_EMPTY :
                result = P1_2_RET_FIFO_EMPTY;
                break;

            case P1_2_OUT_FIFO_PARTIAL :
                result = P1_2_RET_FIFO_PARTIAL;
                break;
...
{
    P1_2_SR_STATUS_MASK &= ((uint8) ~P1_2_INTS_EN_MASK);          /* Clear existing int */
    P1_2_SR_STATUS_MASK |= (interruptSource & P1_2_INTS_EN_MASK); /* Set int */
}


/*******************************************************************************
* Function Name: P1_2_GetIntStatus
********************************************************************************
*
* Summary:
...
{
    return(P1_2_SR_STATUS & P1_2_INTS_EN_MASK);
}


/*******************************************************************************
* Function Name: P1_2_WriteRegValue
********************************************************************************
*
* Summary:
...
{
    CY_SET_REG16(P1_2_SHIFT_REG_LSB_PTR, shiftData);
}


#if(0u != P1_2_USE_INPUT_FIFO)
    /*******************************************************************************
    * Function Name: P1_2_WriteData
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
                                                                         ...
                                                                         
    {
        ...
        if(P1_2_RET_FIFO_FULL != (P1_2_GetFIFOStatus(P1_2_IN_FIFO)))
        {
            CY_SET_REG16(P1_2_IN_FIFO_VAL_LSB_PTR, shiftData);
    ...
    *******************************************************************************/
    ...
    {
        return(CY_GET_REG16(P1_2_OUT_FIFO_VAL_LSB_PTR));
    }
#endif /* (0u != P1_2_USE_OUTPUT_FIFO) */


/*******************************************************************************
* Function Name: P1_2_ReadRegValue
********************************************************************************
*
* Summary:
...
uint16 P1_2_ReadRegValue(void) 
{
    ...
    while(P1_2_RET_FIFO_EMPTY != P1_2_GetFIFOStatus(P1_2_OUT_FIFO))
    {
        (void) CY_GET_REG16(P1_2_OUT_FIFO_VAL_LSB_PTR);
    }

    /* Read of 8 bits from A1 causes capture to output FIFO */
    (void) CY_GET_REG8(P1_2_SHIFT_REG_CAPTURE_PTR);

    /* Read output FIFO */
    result  = CY_GET_REG16(P1_2_OUT_FIFO_VAL_LSB_PTR);
    
    #if(0u != (P1_2_SR_SIZE % 8u))
        result &= ((uint16) P1_2_SR_MASK);
    #endif /* (0u != (P1_2_SR_SIZE % 8u)) */
