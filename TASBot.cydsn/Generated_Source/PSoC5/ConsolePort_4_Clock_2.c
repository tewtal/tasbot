/*******************************************************************************
* File Name: ConsolePort_4_Clock_2.c
* Version 2.20
*
*  Description:
...
#include "ConsolePort_4_Clock_2.h"
...
/*******************************************************************************
* Function Name: ConsolePort_4_Clock_2_Start
********************************************************************************
*
* Summary:
...
void ConsolePort_4_Clock_2_Start(void) 
{
    ...
    ConsolePort_4_Clock_2_CLKEN |= ConsolePort_4_Clock_2_CLKEN_MASK;
	ConsolePort_4_Clock_2_CLKSTBY |= ConsolePort_4_Clock_2_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ConsolePort_4_Clock_2_Stop
********************************************************************************
*
* Summary:
...
void ConsolePort_4_Clock_2_Stop(void) 
{
    ...
    ConsolePort_4_Clock_2_CLKEN &= (uint8)(~ConsolePort_4_Clock_2_CLKEN_MASK);
	ConsolePort_4_Clock_2_CLKSTBY &= (uint8)(~ConsolePort_4_Clock_2_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ConsolePort_4_Clock_2_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ConsolePort_4_Clock_2_CLKEN & ConsolePort_4_Clock_2_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ConsolePort_4_Clock_2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ConsolePort_4_Clock_2_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ConsolePort_4_Clock_2__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ConsolePort_4_Clock_2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ConsolePort_4_Clock_2_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ConsolePort_4_Clock_2_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ConsolePort_4_Clock_2_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ConsolePort_4_Clock_2_GetSourceRegister();
    uint16 oldDivider = ConsolePort_4_Clock_2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ConsolePort_4_Clock_2_CLKEN & ConsolePort_4_Clock_2_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ConsolePort_4_Clock_2_DIV_PTR, clkDivider);
                ConsolePort_4_Clock_2_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ConsolePort_4_Clock_2_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ConsolePort_4_Clock_2_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ConsolePort_4_Clock_2_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ConsolePort_4_Clock_2_DIV_PTR, clkDivider);
				ConsolePort_4_Clock_2_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: ConsolePort_4_Clock_2_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ConsolePort_4_Clock_2_DIV_PTR);
}


/*******************************************************************************
* Function Name: ConsolePort_4_Clock_2_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ConsolePort_4_Clock_2_MOD_SRC |= modeBitMask & (uint8)ConsolePort_4_Clock_2_MODE_MASK;
}


/*******************************************************************************
* Function Name: ConsolePort_4_Clock_2_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ConsolePort_4_Clock_2_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ConsolePort_4_Clock_2_MODE_MASK));
}


/*******************************************************************************
* Function Name: ConsolePort_4_Clock_2_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ConsolePort_4_Clock_2_MOD_SRC & (uint8)(ConsolePort_4_Clock_2_MODE_MASK);
}


/*******************************************************************************
* Function Name: ConsolePort_4_Clock_2_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ConsolePort_4_Clock_2_GetDividerRegister();
    uint8 oldSrc = ConsolePort_4_Clock_2_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ConsolePort_4_Clock_2_MOD_SRC |= CYCLK_SSS;
        ConsolePort_4_Clock_2_MOD_SRC =
            (ConsolePort_4_Clock_2_MOD_SRC & (uint8)(~ConsolePort_4_Clock_2_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ConsolePort_4_Clock_2_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ConsolePort_4_Clock_2_MOD_SRC & ConsolePort_4_Clock_2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ConsolePort_4_Clock_2_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ConsolePort_4_Clock_2_PHASE = clkPhase & ConsolePort_4_Clock_2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ConsolePort_4_Clock_2_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ConsolePort_4_Clock_2_PHASE & ConsolePort_4_Clock_2_PHASE_MASK;
}
