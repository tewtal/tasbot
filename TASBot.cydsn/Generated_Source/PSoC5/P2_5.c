...
uint8 P2_5_initVar = 0u;


/*******************************************************************************
* Function Name: P2_5_Start
********************************************************************************
*
* Summary:
...
*
* Global Variables:
*  P2_5_initVar - used to check initial configuration, modified on
*  first function call.
*
* Reentrant:
*  No.
...
void P2_5_Start(void) 
{
    ...
    {
        P2_5_Init();
        P2_5_initVar = 1u; /* Component initialized */
    }
...
void P2_5_Init(void) 
{
    P2_5_SetIntMode(P2_5_INT_SRC);
}


/*******************************************************************************
* Function Name: P2_5_Stop
********************************************************************************
*
* Summary:
...
void P2_5_Stop(void) 
{
    ...
    P2_5_SR_CONTROL &= ((uint8) ~P2_5_CLK_EN);
    P2_5_DisableInt();
}


/*******************************************************************************
* Function Name: P2_5_EnableInt
********************************************************************************
*
* Summary:
...
void P2_5_EnableInt(void) 
{
    ...
    P2_5_SR_AUX_CONTROL |= P2_5_INTERRUPTS_ENABLE;
    CyExitCriticalSection(interruptState);
}


/*******************************************************************************
* Function Name: P2_5_DisableInt
********************************************************************************
*
* Summary:
...
void P2_5_DisableInt(void) 
{
    ...
    interruptState = CyEnterCriticalSection();
    P2_5_SR_AUX_CONTROL &= ((uint8) ~P2_5_INTERRUPTS_ENABLE);
    CyExitCriticalSection(interruptState);
}


/*******************************************************************************
* Function Name: P2_5_GetFIFOStatus
********************************************************************************
*
* Summary:
...
uint8 P2_5_GetFIFOStatus(uint8 fifoId) 
{
    ...
    result = P2_5_RET_FIFO_NOT_DEFINED;

    #if(0u != P2_5_USE_INPUT_FIFO)
        if(P2_5_IN_FIFO == fifoId)
        {
            switch(P2_5_GET_IN_FIFO_STS)
            {
                case P2_5_IN_FIFO_FULL :
                    result = P2_5_RET_FIFO_FULL;
                    break;

                case P2_5_IN_FIFO_EMPTY :
                    result = P2_5_RET_FIFO_EMPTY;
                    break;

                case P2_5_IN_FIFO_PARTIAL:
                    result = P2_5_RET_FIFO_PARTIAL;
                    break;
    ...
    if(P2_5_OUT_FIFO == fifoId)
    {
        switch(P2_5_GET_OUT_FIFO_STS)
        {
            case P2_5_OUT_FIFO_FULL :
                result = P2_5_RET_FIFO_FULL;
                break;

            case P2_5_OUT_FIFO_EMPTY :
                result = P2_5_RET_FIFO_EMPTY;
                break;

            case P2_5_OUT_FIFO_PARTIAL :
                result = P2_5_RET_FIFO_PARTIAL;
                break;
...
{
    P2_5_SR_STATUS_MASK &= ((uint8) ~P2_5_INTS_EN_MASK);          /* Clear existing int */
    P2_5_SR_STATUS_MASK |= (interruptSource & P2_5_INTS_EN_MASK); /* Set int */
}


/*******************************************************************************
* Function Name: P2_5_GetIntStatus
********************************************************************************
*
* Summary:
...
{
    return(P2_5_SR_STATUS & P2_5_INTS_EN_MASK);
}


/*******************************************************************************
* Function Name: P2_5_WriteRegValue
********************************************************************************
*
* Summary:
...
{
    CY_SET_REG16(P2_5_SHIFT_REG_LSB_PTR, shiftData);
}


#if(0u != P2_5_USE_INPUT_FIFO)
    /*******************************************************************************
    * Function Name: P2_5_WriteData
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
                                                                         ...
                                                                         
    {
        ...
        if(P2_5_RET_FIFO_FULL != (P2_5_GetFIFOStatus(P2_5_IN_FIFO)))
        {
            CY_SET_REG16(P2_5_IN_FIFO_VAL_LSB_PTR, shiftData);
    ...
    *******************************************************************************/
    ...
    {
        return(CY_GET_REG16(P2_5_OUT_FIFO_VAL_LSB_PTR));
    }
#endif /* (0u != P2_5_USE_OUTPUT_FIFO) */


/*******************************************************************************
* Function Name: P2_5_ReadRegValue
********************************************************************************
*
* Summary:
...
uint16 P2_5_ReadRegValue(void) 
{
    ...
    while(P2_5_RET_FIFO_EMPTY != P2_5_GetFIFOStatus(P2_5_OUT_FIFO))
    {
        (void) CY_GET_REG16(P2_5_OUT_FIFO_VAL_LSB_PTR);
    }

    /* Read of 8 bits from A1 causes capture to output FIFO */
    (void) CY_GET_REG8(P2_5_SHIFT_REG_CAPTURE_PTR);

    /* Read output FIFO */
    result  = CY_GET_REG16(P2_5_OUT_FIFO_VAL_LSB_PTR);
    
    #if(0u != (P2_5_SR_SIZE % 8u))
        result &= ((uint16) P2_5_SR_MASK);
    #endif /* (0u != (P2_5_SR_SIZE % 8u)) */
